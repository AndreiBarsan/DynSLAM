...
// TODO(andrei): Consider storing an entire class-conditional distribution in
// each detection
// object.
 ...
 *
 * Yeah, it does make more sense to have a SegmentationAssociationProvider which
 * takes in an
 * InstanceSegmentationResult with its InstanceDetections, and outputs e.g., a
 * list of
 * (InstanceDetection, InstanceReconstruction) pairs, which can then be used for
 * volum. Fusion.
 *
 * The Association Provider can be just an interface, and it could have multiple
 * implementations
 * under the hood, such as overlap-based (with configurable size/time/etc.
 * thresholds),
 * object recognition-based, hybrid, etc.
 *
 * Its job is to be able to associate objects detected in a frame with their
 * appropriate 3D
 * model, or to initialize new models as needed.
 *
 * Then, we could cut out the instance info from the current frame, and give
 * these cut-out bits
 * to the instance rec engine, in their equivalent ProcessFrame method. The only
 * extra necessary
 * thing is the approximate relative pose of the new frame, perhaps in the
 * viewer's frame. Then
 * the instance rec would be able to initialize its alignment system, e.g., ICP,
 * accordingly,
 * based on this initial estimate. We can get this estimate from scene flow.
 *
 * Such an instance reconstructor may also need to keep track of the instance's
 * motion (either
 * in the global frame or the car's frame; we'll see which works best---probably
 * the global
 * frame, since it seems neater), as well as other things, such as (maybe) the
 * motion history,
 * allowing us to render nice tracks in the 3D map, etc.
 */

/// \brief Describes a single object instance detected semantically in an input
/// frame.
...
class InstanceDetection {
 public:
  ...
  /// Class identifier. Depends on the class labels used by the segmentation
  /// pipeline,
  ...
  /// \brief The dataset associated with this instance's detection. Contains
  /// information such
...
/// \brief The result of performing instance-aware semantic segmentation on an
/// input frame.
struct InstanceSegmentationResult {
  /// \brief Specifies the dataset metadata, such as the labels, which are used
  /// by the segmentation.
  const SegmentationDataset* segmentation_dataset;
