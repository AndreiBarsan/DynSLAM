...
float Track::ScoreMatch(const TrackFrame& new_frame) const {
  ...
  // TODO(andrei): Ensure this is modular enough to allow many different
  // matching strategies.
  ...
  // We don't want to accidentally add multiple segments from the same frame to
  // the same track.
  ...
  // e.g., ``car'' and
  // ``truck'' so we may want a more complex reasoning system here in the
  // future.
  ...
  // Using the max makes sure we prefer matching to previous tracks with larger
  // areas if the
  // new detection is also large. This increases robustness to small spurious
  // detections,
  ...
  // If the overlap completely covers one of the frames, then it's considered
  // perfect.
  // Otherwise,	frames which only partially intersect get smaller scores, and
  // frames which don't
  // intersect at all get a score of 0.0.
  float area_score = static_cast<float>(overlap_area) / max_area;

  // Modulate the score by the detection probability. If we see a good overlap
  // but it's a dodgy
  // detection, we may not want to add it to the track. For instance, when using
  // MNC for
  // segmentation, it may sometimes detect both part of a car and the entire car
  // as separate
  // instances. Luckily, in these situations, the proper full detection gets a
  // score near 1.0,
  // while the partial one is around 0.5-0.6. We'd prefer to fuse in the entire
  // car, so we
